...
module.exports = async function sendInvites(context) {
  ...
  const otherOrg = await context.app.get('mongooseClient').model('organisations').findOne({
    createdBy: context.params.user._id
  });
  if (otherOrg) throw new GeneralError('You cannot create more than one organization');
